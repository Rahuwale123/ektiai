...
export function Features() {
  ...
  return (
    <section id="why-aflirt" className="py-24 bg-[#F9FAFB]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-20">
          <h2 className="text-4xl font-bold text-[#1A1A1A] mb-4">Why Connection Matters</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <motion.div
              ...
              className="relative bg-white p-10 rounded-[2.5rem] shadow-sm border border-gray-100 hover:shadow-xl transition-all group"
            >
              {feature.popular && (
                <div className="absolute top-6 right-6 px-3 py-1 rounded-full bg-[#FF5E62] text-white text-[10px] font-bold uppercase tracking-widest">
                  Popular
                </div>
              )}
              <div className={cn("w-14 h-14 rounded-2xl flex items-center justify-center mb-8 group-hover:scale-110 transition-transform", feature.bg)}>
                {feature.icon}
              </div>
